...
public class AccountController {
    ...
    public @ResponseBody
    String getUser(@RequestParam String userid, HttpSession session) throws IOException, NoSuchAlgorithmException {
        if(userid == null)
        ...
        ObjectNode response = om.createObjectNode();
        String loggedInAs = (String) session.getAttribute("username");
        BlockedUser blockedUser = db.getBlockList(account.getUsername());
        if(!loggedInAs.equalsIgnoreCase(account.getUsername()) && 
        		!(blockedUser.blockedUsers.contains(loggedInAs))) {
        ...
        } else if(!loggedInAs.equalsIgnoreCase(account.getUsername()) && 
        		(blockedUser.blockedUsers.contains(loggedInAs))) {
        	response.put("Error", "You cannot view the profile.");
        } else if(loggedInAs.equalsIgnoreCase(account.getUsername())) {
        	response.put("_id", account.get_id());
        	response.put("username", account.getUsername());
        	response.put("firstName", account.getFirstName());
